...
class FingerPushRequest {
	...
	public function request() {
		...
		if (is_null($identity)) { // 일괄발송
			...
			$return = array_merge($result, array('param' => $data));
			...
			$return['step01'] = array_merge($result, array('param' => $param));
			...
			if ($result['result'] == 200 && $result['processCode'] == 20001) {
				...
				foreach ($paramIdentitys as $paramIdentity) {
					...
					$return['step02'][$i] = array_merge($result, array('param' => $param));
					$i++;
				}
			}
			...
			if ($result['result'] == 200 && $result['processCode'] == 20002) {
				...
				$return['step03'] = array_merge($result, array('param' => $param));
			}
		}
